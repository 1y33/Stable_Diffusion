...
class TimeEmbedding(nn.Module):
    def __init__(self,n_embed):
        ...
        self.linear_2 = nn.Linaer(4*n_embed,4*n_embed)
...
class UNET_ResidualBlock(nn.Module):
    def __init__(self,in_ch,out_ch,n_time = 1280):
        super().__init__()
        self.groupnorm = nn.GroupNorm(32,in_ch)
        self.conv_features = nn.Conv2d(in_ch,out_ch,kernel_size=3,padding=1)
        self.linear_time = nn.Linear(n_time,out_ch)

        self.groupnorm_merged = nn.GroupNorm(32,out_ch)
        self.conv_merged = nn.Conv2d(out_ch,out_ch,kernel_size=3,padding=1)
...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self,x,context):
        ...
        residue_short = x
        # Normalization + CrossAtention with skipp conection
